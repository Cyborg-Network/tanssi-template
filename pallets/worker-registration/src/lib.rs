...
use {
    frame_system::{pallet_prelude::*, WeightInfo},
    scale_info::{
...
pub mod crypto {
    use {
        ...
        sp_runtime::{traits::Verify, MultiSignature, MultiSigner},
    };
...
pub mod pallet {
    ...
    pub type TaskAllocations<T: Config> =
        StorageMap<_, Twox64Concat, TaskId, T::AccountId, OptionQuery>;
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn offchain_worker(block_number: BlockNumberFor<T>) {
            ...
            log::debug!(
                "ffchain::worker-registration: Current block: {:?} (parent hash: {:?})",
                block_number,
                parent_hash
            );
            ...
            log::info!(
                "Offchain::worker-registration: ping_key info: {:?}",
                ping_key.clone()
            );
            log::info!(
                "Offchain::worker-registration: oci_mem_ping info: {:?}",
                oci_mem_ping.get::<IndexingData>()
            );
            ...
            if let Ok(Some(data)) = oci_mem_ping.get::<IndexingData>() {
                ...
                if let Some(cluster) = Self::get_worker_clusters(data.1) {
                    ...
                    let response: VerifyClusterJSONResponse =
                        Self::fetch_cluster_status(cluster.ip, cluster.port).unwrap_or_else(|e| {
                            log::error!(
                                "Offchain::worker-registration: fetch_response error: {:?}",
                                e
                            );
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn worker_register(
            ...
            //check ip
                ...
                ip.ipv4.is_some() || ip.ipv6.is_some(),
                Error::<T>::WorkerRegisterMissingIp
            );
            ensure!(port > 0, Error::<T>::WorkerRegisterMissingPort);

            //check cluster
            ensure!(
                WorkerAccounts::<T>::contains_key(creator.clone()) == false,
        ...
        #[pallet::call_index(1)]
        #[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
        pub fn task_scheduler(origin: OriginFor<T>, task_data: String) -> DispatchResult {
            let who = ensure_signed(origin)?;

            ensure!(
                WorkerAccounts::<T>::iter().next().is_some(),
                Error::<T>::NoWorkersAvailable
            );
            ...
            let random_index =
                (sp_io::hashing::blake2_256(&task_data.as_bytes())[0] as usize) % workers.len();
            let selected_worker = workers[random_index].0.clone();

            let cluster_id = Self::get_worker_accounts(selected_worker.clone())
                .ok_or(Error::<T>::WorkerClusterNotRegistered)?;
            ...
            let cluster_id = WorkerAccounts::<T>::get(selected_worker.clone())
                .ok_or(Error::<T>::WorkerClusterNotRegistered)?;
            let cluster = WorkerClusters::<T>::get(cluster_id)
                .ok_or(Error::<T>::WorkerClusterNotRegistered)?;
        ...
        pub fn verify_connection(
            ...
            WorkerClusters::<T>::try_mutate(worker_index, |worker| -> DispatchResult {
                ...
                // Emit an event.
                Self::deposit_event(Event::ConnectionEstablished {
                    cluster_id: worker_index,
                });
                Ok(())
            })?;
        ...
        pub fn submit_completed_task(origin: OriginFor<T>, task_id: TaskId) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let task_owner = TaskOwners::<T>::get(task_id).ok_or(Error::<T>::UnassignedTaskId)?;
            ensure!(task_owner == who, Error::<T>::InvalidTaskOwner);
            ensure!(
                TaskStatus::<T>::get(task_id) == Some(TaskStatusType::Pending),
                Error::<T>::RequirePendingTask
            );
        ...
        #[pallet::call_index(5)]
        #[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
        pub fn unregister_worker(origin: OriginFor<T>, cluster_id: ClusterId) -> DispatchResult {
            let creator = ensure_signed(origin)?;

            //check cluster
            ensure!(
                WorkerAccounts::<T>::get(creator.clone()) == Some(cluster_id),
                Error::<T>::InvalidOwnerOfCluster
            );
            ensure!(
                WorkerClusters::<T>::get(cluster_id) != None,
            ...
            // Emit an event.
            Self::deposit_event(Event::WorkerUnRegistered {
                creator,
                cluster_id,
            });
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn fetch_cluster_status(
            ip: Ip,
            port: u32,
        ) -> Result<VerifyClusterJSONResponse, http::Error> {
